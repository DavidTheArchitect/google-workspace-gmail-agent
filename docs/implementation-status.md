...
- Dynamic local persona and rejection-notice generation with fresh entropy on every attempt.
  ...
  details, expose the internal category label, return a sentence-like persona role, or fall back to
  fixed blocked-sender wording. Creative rejection semantics are governed by the source-specific
  model contract rather than a brittle keyword list. The
